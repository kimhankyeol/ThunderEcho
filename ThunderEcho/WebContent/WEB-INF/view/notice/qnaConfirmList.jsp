...
function page(i){
	var pagenum = i;
	location.href="/counselConfirmList.do?pagenum="+pagenum+"&contentnum=10";
}
//검색함
function page1(i){
	var pagenum = i;
	var classfication="<%=pDTO.getClassfication()%>";
	var userName="<%=nList.get(0).getUserName()%>";
	var mobile="<%=nList.get(0).getMobile()%>";
	var email="<%=nList.get(0).getEmail()%>";
	location.href="/counselConfirmList.do?pagenum="+pagenum+"&contentnum=10&userName="+userName+"&mobile="+mobile+"&email="+email;
}
</script>
...
<div class="subTitleBar">
	<div class="subTitleIn">
		<h2><%=nList.get(0).getUserName()%>님의 문의 목록</h2>
			<ul class="smap">
